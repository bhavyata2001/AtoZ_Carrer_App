...
public class CaCourse extends AppCompatActivity {
    ListView lw1;
    Spinner sp2;
        ...
        sp2.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            ...
            public void onItemSelected(AdapterView<?> adapterView, View view, int position, long l) {
                if (position >= 0 && position < names.length) {
                    ...
                    Toast.makeText(CaCourse.this, "not exist", Toast.LENGTH_LONG).show();
                }
    ...
    private ArrayList<ComicBody> getComicBody() {
        ...
        data.add(new ComicBody("The Institute of Chartered Accountants", 1));

        data.add(new ComicBody("Chartered Institute of Management Accountants (CIMA)", 2));

        data.add(new ComicBody("none", 3));

        data.add(new ComicBody("none", 4));

        data.add(new ComicBody("Lt. Meenaben Jayantilal Kundaliya English Medium Mahila Commerce College", 5));
    ...
    private void getSelectedCategoryData(int catID){
        ...
        if (catID==0)
        {
            Toast.makeText(CaCourse.this,"Choose a right school in your favourite city for your course",Toast.LENGTH_SHORT).show();
        }else {
...
class ComicBody19{
    ...
    public ComicBody19(String name,int catID){
        this.name = name;
        this.catID = catID;
    }
